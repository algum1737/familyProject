...
- 현재 진행 브랜치는 `fix/user-facing-internal-copy`다.
- `main` 기준에서 사용자 노출 내부 구현 문구 정리를 우선 처리했다.
- 이번 브랜치는 아직 `main`에 머지하지 않았다.
...
- 남은 우선 작업은 이 브랜치가 PR/merge된 뒤 기존 backlog였던 Expo route adapter/provider 경계 정리와 Android 최신 빌드 실기기 smoke 확인으로 돌아가면 된다.
