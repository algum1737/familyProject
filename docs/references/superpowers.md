...
이 문서는 Superpowers를 설치한 에이전트 환경에서 familyProject 하네스와 함께 쓰는 방법을 정리한다.
...
| --- | --- | --- |
| `brainstorming` | `docs/product-specs/`, `docs/design-docs/` | 요구사항과 설계 승인 정리 |
| `writing-plans` | `docs/exec-plans/active/` | 실행 가능한 작업 계획 작성 |
| `using-git-worktrees` | `.worktrees/`, 작업 브랜치 | 승인된 구현 작업 격리 |
| `test-driven-development` | 테스트 코드, `docs/QUALITY_SCORE.md` | 실패 테스트 우선 구현 |
...
1. 새 기능이나 행동 변경은 요구사항을 먼저 정리한다.
2. 승인된 설계는 제품 명세, 설계 문서, active exec plan 중 적절한 위치에 남긴다.
3. 구현 계획은 작은 checkbox task로 나누고 파일 경로, 테스트 명령, 기대 결과를 포함한다.
4. 사용자 승인 후 작업 브랜치 또는 격리 worktree에서 구현한다.
5. 기능과 버그 수정은 가능한 한 RED-GREEN-REFACTOR 순서로 진행한다.
6. task 완료 시 요구사항 충족 리뷰와 코드 품질 리뷰를 분리한다.
7. 완료 전 실제 검증 결과를 active plan에 기록한다.
8. 완료된 계획은 `docs/exec-plans/completed/`로 이동하고 `docs/index.md`와 `docs/HANDOFF.md`를 갱신한다.
9. PR 생성 또는 갱신 시 title/body/요약 설명은 `AGENTS.md` 규칙에 따라 한글로 작성한다.
...
현재 환경처럼 skill을 개별 디렉터리로 펼쳐 설치하는 경우에는 `~/.agents/skills/superpowers-systematic-debugging`,
`~/.agents/skills/superpowers-test-driven-development` 같은 경로가 존재하는지 확인한다.
